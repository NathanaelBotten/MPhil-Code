program FindEigenvalue
 ...
 ! integer, parameter :: solving_method = 2 ! Adams Bashforth
  ...
  !open(10, file='CarbonFiniteCoulombVal.dat', access = 'append', status='unknown') 
  !do r_it = n_steps, 500*1e5_I8B
  !   write(10, *) r_it*h_step, FiniteCoulomb(r_it*h_step)
  !end do
  !close(10)

  !!! Reading in Coulomb Potential:
  open(10, file = 'CarbonFiniteCoulombVal.dat', status = 'old')
  do r_it = 1, n_steps
...
!!! Solving for the 2p and 3g states. If you wish to solve for other solutions you might need to adjust the match point in shared.f90
!!! This is due to the wavefunction spreading out as you go to higher angular momentum states
  do state = 2,3 ! num_states
...
subroutine myDerivs(r, params, VC, dydr)
  ...
  !
  integer, parameter :: NuclearPotCheck = 2
  F_local = params(1)
  G_local = params(2)
  lambda  = params(3)
  !
