...
!!! When considering the right side solution, there is a problem where the value for diffrho is so small, such that it returns NaN
!!! To get around this, if this occurs, we set diffrho=0, which is reasonable considering it is a very small number
    if (diffrho /= diffrho) then
       diffrho = 0.0_dp
    end if
...
!!! When considering the right side solution, there is a problem where the value for diff2rho is so small, such that it returns NaN
!!! To get around this, if this occurs, we set diff2rho=0, which is reasonable considering it is a very small number
    if (diff2rho /= diff2rho) then
       diff2rho = 0.0_dp
    end if
...
!!! When considering the right side solution, there is a problem where the value for diffrhosquare is so small, such that it returns NaN
!!! To get around this, if this occurs, we set diffrhosquare=0, which is reasonable considering it is a very small number
    if (diffrhosquare /= diffrhosquare) then
       diffrhosquare = 0.0_dp
    end if
  ...
  function LaplaceRho(rad)
    ...
    LaplaceRho = rho0*(-2.0_dp*a0*(arg+1.0_dp)-rad*(1.0_dp-arg))*arg/(a0**2.0_dp*rad*(arg+1.0_dp)**3.0_dp)
!!! When considering the right side solution, there is a problem where the value for laplacerho is so small, such that it returns NaN
!!! To get around this, if this occurs, we set laplacerho=0, which is reasonable considering it is a very small number
    if (laplacerho /= laplacerho) then
       Laplacerho = 0.0_dp
    end if
...
!!! The nuclear potential as used in the Schrodinger equation Case    
  function NuclearPot(rad)
    ...
    NuclearPot = OM1*rho(rad) + T1*rho(rad) + T2*rho(rad)**2.0_dp + T3*rho(rad)**3.0_dp + drho_drho + drho2_drho + d2rho
...
!!! The potential due to the scalar meson interaction
  function ScalarPot(rad)
    ...
    ScalarPot = w1*Gsig*rho(rad)*(1.0_dp-d*Gsig*rho(rad)+(d*Gsig*rho(rad))**2.0_dp)-w1*Gsig/(msig**2.0_dp)*laplacerho(rad)
...
!!! The potential due to the vector meson interactions
  function VectorPot(rad)
    ...
    vectorPot = wom*Gom*rho(rad)-wom*Gom/(mom**2.0_dp)*laplacerho(rad)
