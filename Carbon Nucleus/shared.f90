module shared
  ...
  real(dp), parameter :: m_Carbon =  12.0_dp*92.161753_dp/fermi                 ! m_carbon (fm) 
  real(dp), parameter :: m_Cascade = 1321.71_dp/fermi                           ! m_Xi (fm) 
  real(dp), parameter :: m_reduced = (m_Cascade*m_Carbon)/(m_Cascade+m_carbon)  ! Reduced Mass (fm)

  real(dp), parameter :: alpha = 0.0072973525376_dp ! fine structure constant
  real(dp), parameter :: Z = 6.0_dp                 ! Proton Number
  real(dp), parameter :: Anum = 12.0_dp             ! Nucleon Number
  ...
  real(dp), parameter :: w2 = 0.3688_dp             !!! Second fitting to to reproduce cascade mass
  real(dp), parameter :: wom = 1.0_dp/3.0_dp        !!! Constnat modifying coupling of omega meson to cascade
...
!!! Charge normalisation, such that int(Charge_dens)=Z
  real(dp), parameter :: ChargeNorm = 10.302296321218574_dp
...
!!! gamma = sqrt(Kappa^2-(Z*alpha)^2)
  ...
  real(dp), parameter :: euler_gamma_func1 = 1.996464838813_dp
  
  real(dp), parameter :: gamma2 = sqrt(4.0_dp-(Z*alpha)**2.0_dp)
  real(dp), parameter :: euler_gamma_func2 = 23.9643756767_dp!
  
  real(dp), parameter :: gamma3 = 2.99399433611_dp 
  real(dp), parameter :: euler_gamma_func3 = 719.1388395684_dp

  real(dp), parameter :: gamma4 = 0.9818360783_dp
  real(dp), parameter :: euler_gamma_func4 = 40278.65507979_dp

  real(dp), parameter :: gamma5 = 4.996398911683_dp 
  real(dp), parameter :: euler_gamma_func5 = 3.625529403360e6_dp 

  real(dp), parameter :: gamma6 =5.996999423434_dp 
  real(dp), parameter :: euler_gamma_func6 =4.786151621118e8_dp 

  !!! Constants for the Woods-Saxon Density
  real(dp), parameter :: R_c = 1.1_dp    ! (fm)
  real(dp), parameter :: a0  = 0.65_dp   ! (fm)
  real(dp), parameter :: rho0 = 0.15_dp  ! (fm)
  
  
  integer(I8B), parameter :: n_steps = 500*1e5_I8B ! number of steps (500 fm)
  integer(I8B), parameter :: match   = 7500*1e3_I8B  ! match point number (7.5 fm)

  integer, parameter :: max_attempts = 1e4  ! max number of attempts to solve

  real(dp), parameter :: h_step  = 1e-5_dp!2_dp  ! step size = 1/100000 fm

  real(dp), parameter :: yErrReq = 1.0e-6_dp      ! required precision (!) 
  !!! Error for integral solvers
  ...
  !!! Number of nodes for the given solution, not used here
  integer, dimension(num_states), parameter :: expected_nodes = (/ 1, 1, 1, 1, 1, 1 /)
